...
func TestSmartDeviceAlert(t *testing.T) {
	...
	assert.Contains(t, lastMessage.Subject, "【磁盘告警】test-system")
	assert.Contains(t, lastMessage.Subject, "S.M.A.R.T. 检测故障")
	assert.Contains(t, lastMessage.Subject, "/dev/sda")
	assert.Contains(t, lastMessage.Text, "Samsung SSD 970 EVO")
	assert.Contains(t, lastMessage.Text, "故障")
}
...
func TestSmartDeviceAlertPassedToWarning(t *testing.T) {
	...
	lastMessage := hub.TestMailer.LastMessage()
	assert.Contains(t, lastMessage.Subject, "【磁盘告警】test-system")
	assert.Contains(t, lastMessage.Subject, "S.M.A.R.T. 检测警告")
	assert.Contains(t, lastMessage.Text, "警告")
}
