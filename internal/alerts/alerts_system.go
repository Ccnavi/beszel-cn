	...
	for _, alert := range validAlerts {
		...
		case "Disk":
			...
			for key, value := range alert.mapSums {
				...
				if sumPct > maxPct {
					maxPct = sumPct
					alert.descriptor = diskDescriptor(key)
				}
			}
		...
		case "Temperature":
			...
			for key, value := range alert.mapSums {
				...
				if sumTemp > maxTemp {
					maxTemp = sumTemp
					alert.descriptor = temperatureDescriptor(key)
				}
			}
...
func (am *AlertManager) sendSystemAlert(alert SystemAlertData) {
	...
	lowAlert := isLowAlert(alert.name)
	subject, body := thresholdAlertMessage(systemName, alert.name, alert.descriptor, alert.unit, alert.val, alert.threshold, alert.min, alert.triggered, lowAlert)
