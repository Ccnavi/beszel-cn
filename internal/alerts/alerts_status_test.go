...
func TestStatusAlertMultipleUsersRespectDifferentMinutes(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		assert.Contains(t, messages[0].Subject, "【告警】shared-system 连接中断")
		...
		assert.Contains(t, messages[1].Subject, "【告警】shared-system 连接中断")
...
func TestStatusAlertMultipleUsersRecoveryBetweenMinutesOnlyAlertsEarlierUser(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		assert.Contains(t, messages[1].Subject, "【恢复】shared-system 已恢复连接")
