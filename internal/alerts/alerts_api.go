...
func (am *AlertManager) SendTestNotification(e *core.RequestEvent) error {
	...
	err := e.BindBody(&data)
	if err != nil || data.URL == "" {
		return e.BadRequestError(notificationURLRequiredMessage(), err)
	}
	...
	if !e.Auth.IsSuperuser() && e.Auth.GetString("role") != "admin" {
		...
		if internalURL {
			return e.ForbiddenError(internalNotificationDestinationMessage(), nil)
		}
	}
	err = am.SendShoutrrrAlert(data.URL, testNotificationTitle(), testNotificationMessage(), am.hub.Settings().Meta.AppURL, testNotificationLinkText())
	if err != nil {
