...
export default function SystemsTable() {
			...
			<CardHeader className="p-0 mb-4">
				<div className="flex flex-col gap-4 w-full">
					<div className="flex flex-col lg:flex-row lg:items-end gap-3">
						<div>
							<CardTitle className="mb-1.5 text-xl sm:text-xl">
						...
						<div className="grid grid-cols-2 gap-1 rounded-md border bg-card p-1 sm:flex lg:ms-auto">
							<StatusFilterButton active={statusFilter === "all"} onClick={() => setStatusFilter("all")}>
								<Trans>All Systems</Trans>
								<span className="tabular-nums text-muted-foreground">{data.length}</span>
							</StatusFilterButton>
							<StatusFilterButton
								active={statusFilter === SystemStatus.Up}
								onClick={() => setStatusFilter(SystemStatus.Up)}
							>
								<span className="size-2 rounded-full bg-green-500" />
								<Trans>Up</Trans>
								<span className="tabular-nums text-muted-foreground">{upSystemsLength}</span>
							</StatusFilterButton>
							<StatusFilterButton
								active={statusFilter === SystemStatus.Down}
								onClick={() => setStatusFilter(SystemStatus.Down)}
							>
								<span className="size-2 rounded-full bg-red-500" />
								<Trans>Down</Trans>
								<span className="tabular-nums text-muted-foreground">{downSystemsLength}</span>
							</StatusFilterButton>
							<StatusFilterButton
								active={statusFilter === SystemStatus.Paused}
								onClick={() => setStatusFilter(SystemStatus.Paused)}
							>
								<span className="size-2 rounded-full bg-amber-500" />
								<Trans>Paused</Trans>
								<span className="tabular-nums text-muted-foreground">{pausedSystemsLength}</span>
							</StatusFilterButton>
						</div>
					</div>

					<div className="flex gap-2 w-full sm:justify-end border-t pt-3">
						<div className="relative flex-1">
							...
							<Input
								...
								value={filter}
								className="h-9 ps-9 pe-10 w-full sm:max-w-80 sm:ms-auto"
							/>
							{filter && (
								<Button
									type="button"
						...
						<DropdownMenu>
							...
							<DropdownMenuContent
								align="end"
								className="max-h-[min(28rem,calc(100dvh-5rem))] w-[min(19rem,calc(100vw-2rem))] overflow-y-auto md:w-auto md:max-w-[calc(100vw-2rem)]"
							>
	...
	return (
		<Card className="w-full rounded-none border-0 bg-transparent shadow-none p-0">
			{CardHead}
			{viewMode === "table" ? (
				// table layout
				<div>
...
function StatusFilterButton({
	...
	return (
		<button
			...
			onClick={onClick}
			className={cn(
				"inline-flex h-8 min-w-0 items-center justify-center gap-1.5 rounded px-2 text-xs font-medium transition-colors sm:h-7 sm:shrink-0 sm:px-2.5",
...
const AllSystemsTable = memo(
	({ table, rows, colLength }: { table: TableType<SystemRecord>; rows: Row<SystemRecord>[]; colLength: number }) => {
		...
		return (
			<div
				className={cn(
					"h-min max-h-[calc(100dvh-14rem)] max-w-full relative overflow-auto border bg-card rounded-md shadow-xs",
...
function SystemsTableHead({ table }: { table: TableType<SystemRecord> }) {
	return (
		<TableHeader className="sticky top-0 z-50 w-full border-b">
		...
		return useMemo(() => {
			return (
				<TableRow
					...
					className={cn("cursor-pointer transition-colors relative safari:transform-3d hover:bg-muted/45", {
			...
			return (
				...
				>
					...
					<CardContent className="text-sm px-5 pt-3.5 pb-4">
						<div className="grid min-w-0 grid-cols-[1.25rem_minmax(4.5rem,max-content)_minmax(0,1fr)] gap-x-2.5 gap-y-2.5">
							{table.getAllColumns().map((column) => {
								...
								return (
									<>
										...
										<div key={`${column.id}-label`} className="flex min-w-0 items-center text-muted-foreground pe-1">
											{name()}:
										</div>
