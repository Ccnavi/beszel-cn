...
export const showLoginFaliedToast = (description = t`Please check your credentials and try again`) => {
	toast({
		...
		description: localizeAuthError(description),
		variant: "destructive",
	})
}
...
function localizeAuthError(message?: string) {
	if (!message) {
		return t`Please check your credentials and try again`
	}
	const normalized = message.toLowerCase()
	if (normalized.includes("failed to authenticate") || normalized.includes("invalid login credentials")) {
		return t`Failed to authenticate`
	}
	if (normalized.includes("invalid email")) {
		return t`Invalid email address.`
	}
	if (normalized.includes("otp")) {
		return t`Invalid or expired one-time password.`
	}
	return message
}
			...
			try {
				...
				if (isFirstRun) {
					...
					if (password !== passwordConfirm) {
						const msg = t`Passwords do not match`
	...
	return (
		<div className={cn("grid gap-6", className)} {...props}>
			...
			{!oauthEnabled && isFirstRun && (
				// only show OAuth helper dialog during onboarding
				<Dialog>
					<DialogTrigger asChild>
						<button type="button" className={cn(buttonVariants({ variant: "outline" }))}>
							<KeyIcon className="me-2 h-4 w-4" />
							<span className="translate-y-px">{t`Configure OAuth`}</span>
						</button>
					</DialogTrigger>
