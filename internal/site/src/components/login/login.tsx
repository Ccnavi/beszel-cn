...
export default function () {
	...
	useEffect(() => {
		...
		pb.send("/api/beszel/first-run", {})
			.then(({ firstRun }) => setFirstRun(firstRun))
	...
	return (
		<div className="min-h-svh grid items-center py-12 safe-area-inset">
			<div
			...
			>
				...
				{loadFailed ? (
					<div
						role="alert"
						className="rounded-md border border-destructive/30 bg-destructive/5 px-4 py-3 text-center text-sm text-muted-foreground"
					>
						{t`Unable to connect to the server. Please refresh the page and try again.`}
					</div>
				) : !authMethods ? (
					<div className="flex h-40 items-center justify-center" role="status" aria-label={t`Loading`}>
