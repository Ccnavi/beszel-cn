...
export default function Navbar() {
	...
	return (
		<div className="flex items-center h-14 md:h-15">
			<Suspense>
			...
			<Link
				href={basePath}
				aria-label={t`Home`}
				className="flex h-full shrink-0 items-center pe-4 me-2 sm:pe-5 sm:me-4 border-e group"
				onMouseEnter={runOnce(() => import("@/components/routes/home"))}
			>
				<Logo className="h-[1.15rem] md:h-[1.15rem] fill-foreground" />
			</Link>
			<Button
				variant="outline"
				className="hidden md:block h-9 min-w-56 text-sm text-muted-foreground px-3 bg-background/60"
			...
			{/* mobile menu */}
			<div className="ms-auto flex h-full items-center gap-0.5 text-xl md:hidden">
				<ModeToggle />
				<Button variant="ghost" size="icon" aria-label={t`Search`} onClick={() => setCommandPaletteOpen(true)}>
			...
			<div
				className="hidden md:flex h-full items-center ms-auto gap-0.5"
				onMouseEnter={() => import("@/components/routes/settings/general")}
			>
				<Tooltip>
				...
				{!isReadOnlyUser() && (
					<Button className="flex h-9 gap-1.5 ms-2" onClick={() => setAddSystemDialogOpen(true)}>
