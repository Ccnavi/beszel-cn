...
## Chinese distribution options

This build defaults to Simplified Chinese for the web UI and alert notifications. Set `VITE_DEFAULT_LOCALE` at frontend build time to change the default UI language, for example `VITE_DEFAULT_LOCALE=en`. Set `BESZEL_ALERT_LOCALE` on the hub runtime to change backend alert notification language, for example `BESZEL_ALERT_LOCALE=en`.
